...
export function heuristicExtract(source: SourceItem): ExtractionResult {
  ...
  for(const line of lines){
    if (/application|apply|submission/i.test(line) && /due|deadline|close[sd]?/i.test(line)) {
      ...
      const date=dateFromText(relevant,source.publishedAt); if(date){
        claims.push({field:'application_deadline',value:combineDateTime(date,timeFromText(relevant)),confidence:.94,evidence:relevant});
      }
    }
    if (/interest meeting|info(?:rmation)? session|coffee chat|recruit(?:ing|ment) event|boot\s?camp|expo/i.test(line)) {
      const date=dateFromText(line,source.publishedAt); const time=timeFromText(line);
      if(date) claims.push({field:'event',value:{title:line.replace(/https?:\/\/\S+/g,'').slice(0,160),startsAt:combineDateTime(date,time),url:extractUrls(line)[0]},confidence:.84,evidence:line});
    }
  }

  for(const url of discoveredUrls){
    if (/forms\.gle|docs\.google\.com\/forms|qualtrics|\/apply/i.test(url)) claims.push({field:'application_url',value:url,confidence:.9,evidence:`Application-like URL: ${url}`});
  }
  const handles=[...text.matchAll(/(?:instagram|ig)\s*[:@]?\s*@?([a-z0-9._]{3,30})/gi)];
  for(const h of handles) claims.push({field:'social_handle',value:{platform:'instagram',handle:h[1]},confidence:.72,evidence:h[0]});
...
export class OpenAIExtractor {
  ...
  async extract(source: SourceItem):Promise<ExtractionResult> {
    ...
    const content:any[]=[{type:'input_text',text:`Source type: ${source.sourceType}\nPublished at: ${source.publishedAt??'unknown'}\nURL: ${source.url??''}\n\n${source.rawText}`}];
    ...
    const body={model:this.model,input:[{role:'system',content:[{type:'input_text',text:
      'Extract recruiting facts only. Never invent dates. Distinguish application deadlines from event dates. If relative dates appear, use published_at to resolve them. Return URLs exactly.'}]},{role:'user',content}],text:{format:{type:'json_schema',name:'recruiting_extraction',strict:true,schema:{type:'object',additionalProperties:false,properties:{claims:{type:'array',items:{type:'object',additionalProperties:false,properties:{field:{type:'string',enum:['application_open','application_deadline','application_url','event','requirement','status','social_handle','recruiting_note']},value:{},confidence:{type:'number'},evidence:{type:'string'}},required:['field','value','confidence','evidence']}},discoveredUrls:{type:'array',items:{type:'string'}}},required:['claims','discoveredUrls']}}}};
    ...
    const outputText=json.output_text ?? json.output?.flatMap((o:any)=>o.content??[]).find((x:any)=>x.type==='output_text')?.text;
