...
export class Store implements RecruitingRepository {
  ...
  ensureTenant(id: string, name: string): void {
    this.db.prepare('insert or ignore into tenants(id,name,created_at) values(?,?,?)')
      .run(id, name, nowIso());
  }

  async upsertOrganization(organization: Organization, tenantId = organization.tenantId ?? this.defaultTenantId): Promise<void> {
    ...
    on conflict(tenant_id,id) do update set
      ...
      organization.instagramHandle ?? null, organization.linkedinUrl ?? null, now, now,
    );
  }

  async getOrganization(id: string, tenantId = this.defaultTenantId): Promise<Organization | undefined> {
    const row = this.db.prepare('select * from organizations where tenant_id=? and id=?')
  ...
  async listOrganizations(tenantId = this.defaultTenantId): Promise<Organization[]> {
    return (this.db.prepare('select * from organizations where tenant_id=? order by name')
      .all(tenantId) as Record<string, unknown>[]).map((row) => this.mapOrganization(row));
  }

  async stageSource(source: SourceItem, tenantId = source.tenantId ?? this.defaultTenantId): Promise<StageSourceResult> {
    const identity = sourceIdentity(source);
    const sourceId = stableId('src', `${tenantId}:${source.organizationId}:${source.sourceType}:${identity}`);
    ...
    return this.transaction(async () => {
      ...
      on conflict(tenant_id,organization_id,source_type,identity_key) do update set
        ...
        .run(sourceId, tenantId, source.organizationId, source.sourceType, identity,
          source.externalId ?? null, source.url ?? null, source.title ?? null, now, now);

      this.db.prepare(`insert or ignore into source_versions(
        ...
        versionId, sourceId, tenantId, source.organizationId, hash, source.rawText,
        JSON.stringify(source.media), source.publishedAt ?? null, source.fetchedAt,
        JSON.stringify(source.metadata ?? {}), 'received', now,
      );

      const row = this.db.prepare('select status,attempt_count from source_versions where tenant_id=? and id=?')
      ...
      if (shouldProcess) {
        this.db.prepare(`update source_versions set status='queued',next_attempt_at=null where tenant_id=? and id=?`)
          .run(tenantId, versionId);
      }
      return {
        sourceId, versionId, status: shouldProcess ? 'queued' : row.status, shouldProcess,
        unchanged: row.status !== 'received', attemptCount: row.attempt_count,
      };
    });
  }
  ...
  async markSourceProcessing(versionId: string, tenantId = this.defaultTenantId): Promise<number> {
    const result = this.db.prepare(`update source_versions
    ...
    const row = this.db.prepare('select attempt_count from source_versions where tenant_id=? and id=?')
  ...
  async markSourceSucceeded(versionId: string, tenantId = this.defaultTenantId): Promise<void> {
    this.db.prepare(`update source_versions set status='succeeded',completed_at=?,next_attempt_at=null,last_error=null
      where tenant_id=? and id=?`).run(nowIso(), tenantId, versionId);
  }

  async markSourceFailed(versionId: string, failure: ProcessingFailure, tenantId = this.defaultTenantId): Promise<void> {
    const status: SourceVersionStatus = failure.retryable ? 'retryable_failed' : 'terminal_failed';
    this.db.prepare(`update source_versions set status=?,last_error=?,next_attempt_at=?,completed_at=?
      where tenant_id=? and id=?`).run(
      status, failure.message.slice(0, 500), failure.nextAttemptAt ?? null, nowIso(), tenantId, versionId,
    );
  }

  async getSource(id: string, tenantId = this.defaultTenantId): Promise<SourceItem | undefined> {
    const row = this.db.prepare(`select v.id as version_id,v.organization_id,s.source_type,s.external_id,
    ...
    return {
      id: String(row.version_id), tenantId: String(row.tenant_id),
      organizationId: String(row.organization_id), sourceType: String(row.source_type) as SourceType,
      externalId: row.external_id ? String(row.external_id) : undefined,
      url: row.url ? String(row.url) : undefined, title: row.title ? String(row.title) : undefined,
      rawText: String(row.raw_text), media: parseJsonSafe(String(row.media), []),
      publishedAt: row.published_at ? String(row.published_at) : undefined,
      fetchedAt: String(row.fetched_at), metadata: parseJsonSafe(String(row.metadata), {}),
    };
  }
  ...
  async putClaims(claims: Claim[], tenantId = this.defaultTenantId): Promise<void> {
    ...
    for (const claim of claims) statement.run(
  ...
  async listClaims(organizationId: string, tenantId = this.defaultTenantId): Promise<Claim[]> {
    const rows = this.db.prepare(`select * from claims where tenant_id=? and organization_id=?
      order by coalesce(published_at,extracted_at)`).all(tenantId, organizationId) as Record<string, unknown>[];
    return rows.map((row) => ({
      id: String(row.id), tenantId: String(row.tenant_id), organizationId: String(row.organization_id),
      sourceItemId: String(row.source_version_id), field: String(row.field) as Claim['field'],
      value: parseJsonSafe(String(row.value), null), confidence: Number(row.confidence),
      publishedAt: row.published_at ? String(row.published_at) : undefined,
      extractedAt: String(row.extracted_at), supersedes: parseJsonSafe(String(row.supersedes), []),
      evidence: row.evidence ? String(row.evidence) : undefined,
    }));
  }

  async replaceOpportunities(organizationId: string, opportunities: Opportunity[], tenantId = this.defaultTenantId): Promise<void> {
    this.db.prepare('delete from opportunities where tenant_id=? and organization_id=?')
    ...
    for (const opportunity of opportunities) statement.run(
      ...
      opportunity.confidence, opportunity.stale ? 1 : 0, JSON.stringify(opportunity.sourceClaimIds),
      opportunity.explanation, opportunity.resolverVersion ?? 'resolver-v1', opportunity.resolvedAt,
    );
  }

  async listOpportunities(organizationId?: string, tenantId = this.defaultTenantId): Promise<Opportunity[]> {
    const rows = (organizationId
      ? this.db.prepare(`select * from opportunities where tenant_id=? and organization_id=?
          order by coalesce(deadline_at,starts_at)`).all(tenantId, organizationId)
      : this.db.prepare(`select * from opportunities where tenant_id=?
          order by coalesce(deadline_at,starts_at)`).all(tenantId)) as Record<string, unknown>[];
    return rows.map((row) => ({
      id: String(row.id), tenantId: String(row.tenant_id), organizationId: String(row.organization_id),
      kind: String(row.kind) as Opportunity['kind'], title: String(row.title),
      deadlineAt: row.deadline_at ? String(row.deadline_at) : undefined,
      startsAt: row.starts_at ? String(row.starts_at) : undefined,
      url: row.url ? String(row.url) : undefined, role: row.role ? String(row.role) : undefined,
      confidence: Number(row.confidence), stale: Boolean(row.stale),
      sourceClaimIds: parseJsonSafe(String(row.source_claim_ids), []),
      explanation: String(row.explanation), resolverVersion: String(row.resolver_version),
      resolvedAt: String(row.resolved_at),
    }));
  }

  async getConnectorState(connector: string, scope: string, tenantId = this.defaultTenantId): Promise<{
    cursor?: string; metadata: Record<string, unknown>;
  }> {
    const row = this.db.prepare(`select * from connector_state where tenant_id=? and connector=? and scope=?`)
  ...
  async setConnectorState(
    connector: string, scope: string, cursor?: string,
    metadata: Record<string, unknown> = {}, tenantId = this.defaultTenantId,
  ): Promise<void> {
    this.db.prepare(`insert into connector_state(tenant_id,connector,scope,cursor,metadata,updated_at)
  ...
  async getSourceVersionStatus(versionId: string, tenantId = this.defaultTenantId): Promise<{
    status: SourceVersionStatus; attemptCount: number; lastError?: string;
  } | undefined> {
    const row = this.db.prepare(`select status,attempt_count,last_error from source_versions
      where tenant_id=? and id=?`).get(tenantId, versionId) as Record<string, unknown> | undefined;
    return row ? {
  ...
  private mapOrganization(row: Record<string, unknown>): Organization {
    return {
      ...
      school: String(row.school), heelLifeUrl: row.heel_life_url ? String(row.heel_life_url) : undefined,
