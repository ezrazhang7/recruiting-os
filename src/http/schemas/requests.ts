...
export const developmentLoginSchema = z
  .object({ email: z.string().email(), displayName: z.string().min(1).max(120).optional() })
  .strict();

export function validateScreenshotBytes(
  base64: string,
  mimeType: string,
  maxBytes: number,
): Buffer {
  const bytes = Buffer.from(base64, 'base64');
  if (bytes.length === 0 || bytes.length > maxBytes) throw new Error('Screenshot size is invalid');
  ...
  if (!signatures[mimeType]?.(bytes))
    throw new Error('Screenshot bytes do not match the declared type');
  return bytes;
}
