...
export class SessionService {
  ...
  async issue(
    ...
    const authenticated = await this.repository.authenticateSession(hashSecret(token));
    if (!authenticated) throw new Error('Created session could not be loaded');
    return { token, csrfToken, expiresAt, principal: authenticated.principal };
  }
  async authenticate(token: string | undefined): Promise<SessionAuthentication | undefined> {
    return token ? this.repository.authenticateSession(hashSecret(token)) : undefined;
  }
  verifyCsrf(authentication: SessionAuthentication, provided: string | undefined): boolean {
    if (!provided) return false;
    const actual = Buffer.from(hashSecret(provided));
    const expected = Buffer.from(authentication.csrfHash);
    return actual.length === expected.length && timingSafeEqual(actual, expected);
  }
  async revoke(principal: AuthPrincipal): Promise<void> {
    await this.repository.revokeSession(principal.sessionId, principal.tenantId);
  }
}
