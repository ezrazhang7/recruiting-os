...
export class InstagramConnector {
  ...
  private async get(url: string): Promise<any> {
    ...
    if (!r.ok) throw new Error(`Instagram ${r.status}: ${await r.text()}`);
    return r.json();
  }
  async businessDiscovery(
    handle: string,
    limit = 25,
  ): Promise<{ profile: any; sources: SourceItem[] }> {
    ...
    if (!p) throw new Error(`No business_discovery result for @${h}`);
    const sources: SourceItem[] = (p.media?.data ?? []).map((m: any) => {
      const media: MediaRef[] = [];
      const add = (x: any) => {
        const url = x.media_type === 'VIDEO' ? x.thumbnail_url || x.media_url : x.media_url;
        if (url) media.push({ type: 'image', url });
      };
      add(m);
      for (const c of m.children?.data ?? []) add(c);
      return {
        id: stableId('src', `instagram:${m.id}`),
        organizationId: '',
        ...
        fetchedAt: nowIso(),
        metadata: { handle: h, mediaType: m.media_type },
      };
    });
    return { profile: p, sources };
  }
  async sourcesForOrganization(orgId: string, handle: string, limit = 25) {
    const x = await this.businessDiscovery(handle, limit);
    return { profile: x.profile, sources: x.sources.map((s) => ({ ...s, organizationId: orgId })) };
  }
}
