...
export class GroupMeConnector {
  constructor(private token:string,private http:HttpClient=new ProviderHttpClient(new Set(['api.groupme.com'])).fetch){}
  ...
  async syncGroup(store:RecruitingRepository,orgId:string,groupId:string,tenantId?:string):Promise<SourceItem[]>{
    const state=await store.getConnectorState('groupme',groupId,tenantId); const msgs=await this.messages(groupId,state.cursor); const out:SourceItem[]=[];
    ...
    if(sorted.length) await store.setConnectorState('groupme',groupId,String(sorted.at(-1).id),{},tenantId);
    return out;
  }
}
