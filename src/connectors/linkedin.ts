...
export class LinkedInConnector {
  constructor(private token:string,private version='202608',private http:HttpClient=new ProviderHttpClient(new Set(['api.linkedin.com'])).fetch){}
  ...
  async posts(orgId:string,organizationUrn:string,count=50):Promise<SourceItem[]>{
    ...
    for(const p of x.elements??[]){
      ...
      for(const urn of imageUrns){ try{ const img=await this.get(`https://api.linkedin.com/rest/images/${encodeURIComponent(urn)}`); const url=img.downloadUrl ?? (img.downloadUrlExpiresAt ? img.downloadUrl : undefined); if(url)media.push({type:'image',url}); }catch{ /* A missing image must not discard the post text. */ } }
