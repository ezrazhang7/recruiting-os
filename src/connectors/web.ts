...
function linksFromHtml(html:string,base:string):string[] {
  const out:string[]=[]; for(const m of html.matchAll(/<a\b[^>]*href=["']([^"']+)["'][^>]*>/gi)){ const href=m[1]; if(!href)continue; try{ out.push(new URL(decodeEntities(href),base).toString()); }catch{ /* Ignore malformed page links. */ } }
  return [...new Set(out)];
}
function titleFromHtml(html:string):string|undefined { const m=html.match(/<title[^>]*>([\s\S]*?)<\/title>/i); return m?.[1]?stripTags(m[1]):undefined; }

export class WebConnector {
  constructor(private http:HttpClient=new SafeHttpClient().fetch, private validate:(u:string)=>Promise<URL>=assertPublicHttpUrl){}
  async fetchSource(orgId:string,url:string,sourceType?:SourceType):Promise<{source:SourceItem;links:string[]}> {
    ...
    try{
      const r=await this.http(url,{redirect:'manual',signal:ctrl.signal,headers:{'user-agent':'RecruitingOS/0.2 (+campus recruiting aggregator)'}});
