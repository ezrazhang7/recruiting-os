...
test('180DC stale Spring website + closed form is not presented as current',async()=>{
  const st=new Store();st.upsertOrganization({id:'180dc-unc',name:'180 Degrees Consulting at UNC',school:'UNC',websiteUrl:'https://unc180dc.wixsite.com/home/join-us',instagramHandle:'unc180dc'});const ing=new IngestionService(st,fallbackExtractor);
  ...
  await ing.ingest(website,{followLinks:false});await ing.ingest(form,{followLinks:false});const app=resolveOrganization(st,'180dc-unc',new Date('2026-08-27T12:00:00Z')).find(o=>o.kind==='application');
  assert.equal(app?.deadlineAt,'2026-01-16T23:59:00-04:00');assert.equal(app?.stale,true);assert.match(app?.explanation??'',/closed|past/i);st.close();
});

test('180DC newer social open announcement suppresses stale Spring deadline',async()=>{
  const st=new Store();st.upsertOrganization({id:'180dc-unc',name:'180 Degrees Consulting at UNC',school:'UNC'});const ing=new IngestionService(st,fallbackExtractor);
  ...
  const app=resolveOrganization(st,'180dc-unc',new Date('2026-08-27T12:00:00Z')).find(o=>o.kind==='application');assert.equal(app?.deadlineAt,undefined);assert.equal(app?.stale,false);assert.match(app?.explanation??'',/suppressed/i);st.close();
});
