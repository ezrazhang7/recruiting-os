...
test('private, metadata, mapped, documentation, and carrier NAT IPs are blocked', () => {
  for (const address of [
    ...
    '100.64.0.1', '192.0.2.1', '198.51.100.1', '203.0.113.1', '::1', 'fc00::1',
...
test('unsafe schemes, credentials, and ports are rejected', async () => {
  ...
  await assert.rejects(() => resolvePublicHttpTarget('https://user:pass@example.com'), /Credentialed/);
...
test('streamed response bodies are capped', async () => {
  const client = new SafeHttpClient({
    maxResponseBytes: 3,
    resolver: async (raw) => ({ url: new URL(raw), addresses: [{ address: '8.8.8.8', family: 4 }] }),
