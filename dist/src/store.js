...
class Store {
    ...
    listOrganizations() {
        return this.db.prepare('select * from organizations order by name').all().map(r => ({
    ...
    putClaims(claims) {
      ...
      values(?,?,?,?,?,?,?,?,?,?)`);
        for (const c of claims)
            q.run(c.id, c.organizationId, c.sourceItemId, c.field, JSON.stringify(c.value), c.confidence, c.publishedAt ?? null, c.extractedAt, JSON.stringify(c.supersedes ?? []), c.evidence ?? null);
    }
    listClaims(orgId) {
        return this.db.prepare('select * from claims where organization_id=? order by coalesce(published_at,extracted_at)').all(orgId).map(r => ({
            id: r.id, organizationId: r.organization_id, sourceItemId: r.source_item_id, field: r.field, value: (0, util_1.parseJsonSafe)(r.value, null), confidence: r.confidence, publishedAt: r.published_at ?? undefined, extractedAt: r.extracted_at, supersedes: (0, util_1.parseJsonSafe)(r.supersedes, []), evidence: r.evidence ?? undefined
        }));
    }
    getSource(id) {
        const r = this.db.prepare('select * from source_items where id=?').get(id);
        if (!r)
            return undefined;
        return { id: r.id, organizationId: r.organization_id, sourceType: r.source_type, externalId: r.external_id ?? undefined, url: r.url ?? undefined, title: r.title ?? undefined, rawText: r.raw_text, media: (0, util_1.parseJsonSafe)(r.media, []), publishedAt: r.published_at ?? undefined, fetchedAt: r.fetched_at, metadata: (0, util_1.parseJsonSafe)(r.metadata, {}) };
    }
    replaceOpportunities(orgId, xs) {
      ...
      values(?,?,?,?,?,?,?,?,?,?,?,?,?)`);
        for (const x of xs)
            q.run(x.id, x.organizationId, x.kind, x.title, x.deadlineAt ?? null, x.startsAt ?? null, x.url ?? null, x.role ?? null, x.confidence, x.stale ? 1 : 0, JSON.stringify(x.sourceClaimIds), x.explanation, x.resolvedAt);
    }
    listOpportunities(orgId) {
        const rows = (orgId ? this.db.prepare('select * from opportunities where organization_id=? order by coalesce(deadline_at,starts_at)').all(orgId) : this.db.prepare('select * from opportunities order by coalesce(deadline_at,starts_at)').all());
        return rows.map(r => ({ id: r.id, organizationId: r.organization_id, kind: r.kind, title: r.title, deadlineAt: r.deadline_at ?? undefined, startsAt: r.starts_at ?? undefined, url: r.url ?? undefined, role: r.role ?? undefined, confidence: r.confidence, stale: !!r.stale, sourceClaimIds: (0, util_1.parseJsonSafe)(r.source_claim_ids, []), explanation: r.explanation, resolvedAt: r.resolved_at }));
    }
    getConnectorState(connector, scope) {
        const r = this.db.prepare('select * from connector_state where connector=? and scope=?').get(connector, scope);
        return r ? { cursor: r.cursor ?? undefined, metadata: (0, util_1.parseJsonSafe)(r.metadata, {}) } : { metadata: {} };
    }
    setConnectorState(connector, scope, cursor, metadata = {}) {
